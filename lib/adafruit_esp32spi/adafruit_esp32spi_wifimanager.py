# Mikejc58 -    Changed to print Firmware Version nicely
#               Changed to print MAC address bytes in proper order
#               Changed to add SSID to "Connecting to AP ..." message
...
class ESPSPI_WiFiManager:
    ...
    def connect(self):
        ...
        if self.debug:
            ...
            print("Firmware vers.", self.esp.firmware_version.decode())
            mac = self.esp.MAC_address
            print("MAC Addr: {:02X}:{:02X}:{:02X}:{:02X}:{:02X}:{:02X}".format(mac[5], mac[4], mac[3], mac[2], mac[1], mac[0]))
    ...
    def connect_normal(self):
        ...
        while not self.esp.is_connected:
            try:
                ...
                print("Failed to connect, retrying  ", error)
                failure_count += 1
